      ...
      .then(async (data) => {
        ...
        result.push(
          ...
          ...sntDataObj.row.filter((data) => {
            // 검색어 반영
            if (
              ![data.FACLT_NM, data.REFINE_LOTNO_ADDR, data.REFINE_ROADNM_ADDR, data.COPRTN_GRP_NM].some(
                (field) => field?.includes(searchText)
              )
            )
              return false;
            ...
            if (detailCategory !== "전체" && !data.FACLT_KIND_NM.includes(detailCategory)) return false;
...
// export const getWelfareServiceAPI: GetServiceFacilityAPI = ({ SIGName }) => {
...
export const getWelfareServiceAPI: GetServiceFacilityAPI = async (param) => {
  const { SIGName, searchText, page, notiCount, detailCategory, profit } = param; // param

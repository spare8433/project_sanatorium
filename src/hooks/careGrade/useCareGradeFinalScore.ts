...
const useCareGradeFinalScore = (ScoreObj: CareGradeTotalScore) => {
  const { recognPart, behaviorPart, nursingPart, rehabExercisePart } = ScoreObj
  ...
  const getExceptionCriteria = useCallback(() => {
    const exceptionNum = Math.exp(
      -27.0 +
        1.37 * recognPart.dateRecognition +
  ...
  }, [recognPart, behaviorPart, nursingPart, convertedScore])

  /** 치매 의심 여부 반환 함수 */
  const getSuspectedDementia = useCallback(() => {
    return convertedScore.behaviorPart + convertedScore.recognPart >= 7
  }, [convertedScore.behaviorPart, convertedScore.recognPart])

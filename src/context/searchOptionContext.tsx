...
type DetailFacilityCategory = ServiceFacilityCategory | SanatoriumFacilityCategory | "전체";
type ProfitType = "영리" | "비영리" | "전체";
type GradeType = HospitalGrade | "전체";
...
interface ContextType {
  states: {
    ...
    city: string;
    province: string;
    detailCtg: DetailFacilityCategory;
    profit: ProfitType;
    grade: GradeType;
    p: number;
  };
  setStates: {
    ...
    setDetailCtg: React.Dispatch<React.SetStateAction<DetailFacilityCategory>>;
...
const SearchOptionProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const [detailCtg, changeDetailCtg, setDetailCtg] = useInput<DetailFacilityCategory>(
    (getParameter("detailCtg") ?? "전체") as DetailFacilityCategory
  );

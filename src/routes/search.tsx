...
const Search = ({ location }) => {
  ...
  const makePagenation = useCallback((_current, _pageCount, _start, _last) => {
    ...
    if (query?.type) previous += `type=${query.type}`
    if (query?.city) previous += `&city=${query.city}`
    if (query?.search_text) previous += `&search_text=${query.search_text}`
    ...
    if (_pageCount >= _last) limit = _last
    else limit = _pageCount
        ...
        .then(function (response) {
          ...
          if (_json_title === 0) {
            ...
            result = searchedData.slice(slice_start, slice_end).map((row, index) => (
              <div
                key={'searched_faclt_' + index}
                className="item"
                onClick={() => {
                  compose_modal(row, _json_title)
                  handleShow()
                }}
              >
    ...
    <div className="Search">
      <div className="Main_content">
        <div className="containor_box">
          <div className="containor_content">{!!search_item.length && search_item}</div>
          <div className="pagination">{!!pagenationCode.length && pagenationCode}</div>
        </div>
      </div>
      <Modal show={show} onHide={handleClose} backdrop="static" keyboard={false}>
        ...
        <Modal.Body>{modalCode}</Modal.Body>
        <Modal.Footer>

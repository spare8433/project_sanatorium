...
const CareGradeList = () => {
  ...
  const { physicalScore, recognitionScore, behaviorScore, nursingScore, rehabExerciseScore, rehabJointScore } = states;
  ...
  const handleScoreResult = useCallback(() => {
    if (
      ...
      Object.values(recognitionScore).includes(-1) ||
  ...
  }, [physicalScore, recognitionScore, behaviorScore, nursingScore, rehabExerciseScore, rehabJointScore]);

  const GradeCatoeryItem = ({ mode, children }: { mode: CareGradeModeType; children: ReactNode }) => (
    ...
    <ModalContainor show={true} keyboard={false} centered={true} size="xl" scrollable={true}>
      ...
      <Modal.Body>
        <ListContentBox>
          ...
          <GradeCatoeryItem mode="recognition">
            인지기능 영역 &nbsp;&nbsp;{checkProgress(recognitionScore)}
          </GradeCatoeryItem>

...
import { REHAB_EXERCISE_ANSWERS, REHAB_EXERCISE_QUESTIONS } from "@constants/careGradeQuestions";
import CareGradeContext from "@context/careGradeContext";
import { useCallback, useContext, useState } from "react";
import { Button, Modal } from "react-bootstrap";
import { useNavigate } from "react-router-dom";

const RehabExerciseTest = () => {
  const navigate = useNavigate();
  const { rehabExerciseResponse, updateMode, updateRehabExerciseResponse } = useContext(CareGradeContext);
  const [current, setCurrent] = useState(0);
  const [answerScore, setAnswerScore] = useState(-1);

  const scoreKeyArr = REHAB_EXERCISE_QUESTIONS.map(({ category }) => category);
  ...
  const handleNextQuestion = useCallback(() => {
    ...
    setAnswerScore(rehabExerciseResponse[scoreKeyArr[current + 1]]);
  }, [current, rehabExerciseResponse, scoreKeyArr, updateMode]);
  ...
  const AnswerItems = () => {
    return REHAB_EXERCISE_ANSWERS.map(({ answer, score }, key) => (
        ...
        <label>
          <input
            ...
            onChange={() => {
              ...
              updateRehabExerciseResponse(scoreKeyArr[current], score);
            }}
          />
          {answer}
        </label>
    ...
    <ModalContainer show={true} keyboard={false} centered={true} size="xl" scrollable={true}>
      ...
      <Modal.Body>
        <QuestionBox>
          ...
          <h2>{REHAB_EXERCISE_QUESTIONS[current].question}</h2>
          <AnswerList>
            <AnswerItems />
          </AnswerList>
        </QuestionBox>
...
export default RehabExerciseTest;

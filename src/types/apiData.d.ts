...
/** 요양시설 상세 데이터 타입 */
export type SanatoriumDetailData = {
  NO: number // 번호
...
/** 재가 노인복지시설 상세 데이터 타입 */
export type WelfareServiceDetailData = {
  SIGUNGU_NM: string // 시군구명
  FACLT_KIND_NM: string // 시설종류
  FACLT_NM: string // 시설명
  LNGTR_RECPER_APPONT_INST_YN_NM: string | null //장기요양지정기관 여부
...
/** 요양병원 API 결과 객체 */
export type HospitalAPIObj = {
  Hosptlevaltnrcper: [
    CommonAPIObjHead,
    {
      row: HospitalDetailData[]
    },
  ]
}

/** 요양시설 API 결과 객체 */
export type SanatoriumAPIObj = {
  OldpsnMedcareWelfac: [
    CommonAPIObjHead,
    {
      row: SanatoriumDetailData[]
    },
  ]
}

/** 재가 노인복지시설 API 결과 객체 */
export type WelfareServiceAPIObj = {
  HtygdWelfaclt: [
    CommonAPIObjHead,
    {
      row: WelfareServiceDetailData[]
    },
  ]
}
